...
- 两个私有样本均未检测到 Xref 定义或模型空间 Xref 引用，因此不能把“未发现 Xref”表述为 Xref 保真。受控合成 Xref 样本已验证附件与 Overlay 的检测，并在引用目标缺失时产生稳定诊断；它未验证加载、嵌入或保真转换 Xref 内容。第一个私有样本含代理实体，ezdxf 可只读提取其代理图形且未出现提取失败，但原始对象语义仍未证明可保留。
...
- 当前决定为 `continue-validation`，不是已接受依赖或正式 DXF 支持；仍需 Xref 内容加载/保真、代理对象语义保真、实体覆盖、取消/资源限制以及上述契约缺口的设计决策和运行环境证据。
